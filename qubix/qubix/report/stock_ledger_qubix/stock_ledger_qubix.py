...
def get_columns():
	columns = [
		...
		{"label": _("QCR No"), "fieldname": "qcr_no", "fieldtype": "Link", "options": "Quality Inspection", "width": 150},
...
def get_stock_ledger_entries(filters, items):
	...
	sl_entries = frappe.db.sql("""
		SELECT
			...
			sle.voucher_no,
			sle.batch_no,
			(CASE WHEN qi.name != NULL THEN qi.name ELSE pri.quality_inspection END) as qcr_no,
			sle.serial_no,
			sle.company,
			sle.project,
		...
		ON sle.voucher_no = qi.reference_name
		LEFT JOIN `tabPurchase Receipt Item` pri
		ON sle.batch_no = pri.batch_no AND pri.parenttype="Purchase Receipt"
		WHERE
